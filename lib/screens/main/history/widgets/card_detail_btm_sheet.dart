...
import 'package:mobile_ultra/screens/main/history/widgets/card_detail_footer.dart';
import 'package:mobile_ultra/screens/main/history/widgets/card_detail_header.dart';
import 'package:mobile_ultra/screens/main/history/widgets/card_payment_detail.dart';
...
class CardDetailBtmSheet extends StatefulWidget {
  ...
  @override
  _CardDetailBtmSheetState createState() => _CardDetailBtmSheetState();
}

class _CardDetailBtmSheetState extends State<CardDetailBtmSheet>
    with CardDetailView {
  String? receipt;
  bool identificationLoading = false;
  h.HistoryResponse? transaction;
  ...
  void initState() {
    super.initState();
    data = widget.historyItem?.date7 == null
        ? null
    ...
    transaction = widget.historyItem;
    if (transaction != null &&
        transaction is h.HistoryResponse &&
        (transaction?.isSuccess ?? false)) {
      CardDetailPresenter(this).getReceipt(
          [widget.historyItem?.merchantHash ?? '-1', locale.prefix]);
    }
  }

  @override
  Widget build(BuildContext context) {
    final hasAddButton = widget.historyItem!.billId != null &&
        !favoriteList.any((e) => e.billId == widget.historyItem!.billId);

    final query = MediaQuery.of(context);
    final screenHeight = query.size.height;
    final dialogHeight =
        identificationLoading ? screenHeight / 2 : screenHeight * 0.9;
                ...
                child: Column(
                  ...
                  children: [
                    CardDetailHeader(
                      ...
                      status: widget.historyItem!.status ?? 'ROK',
                      type: widget.historyItem!.tranType!,
                      dateTime: "${outputFormat.format(data)} "
                          "${data == null ? '' : DateFormat("HH:mm").format(data!)}",
                      price: (widget.historyItem?.amount ?? 0) / 100,
                      merchantHash: widget.historyItem?.merchantHash,
                      card1: widget.historyItem?.card1 ?? '',
                      tranType: widget.historyItem?.tranType ?? '',
                      pan: widget.historyItem?.pan ?? '',
                    ),
                    ...
                    Flexible(
                      child: ListView(
                        ...
                        children: [
                          ...
                          CardDetailFooter(
                              onTaps: onAttemptToAddFavorite,
                              hasAddButton: hasAddButton,
                              hasDownloadButton: buttonActive,
                              onTap: () async {
                                setState(() {
                                  identificationLoading = true;
                                  saveReceiptToPdf(this.receipt ??
                                      transaction?.paymentReceipt['receipt']);
                                });
                                Platform.isAndroid
  ...
  void onAttemptToAddFavorite() async {
    FavoritePresenter.newFavorite(
      data: {
        'billId': widget.historyItem!.billId,
        'name': "",
      },
  ...
  saveReceiptToPdf(String receipt) async {
    ...
    if (permissionStatus.isPermanentlyDenied) {
      showModalBottomSheet(
        ...
        builder: (_) => OpenAppSettingsBottomSheet(
          request: PermissionRequest.storage(),
  ...
  void onSaveCheck() async {
    ...
    if (result != null) {
      ...
      onShowMessage(
        ...
        locale.getText('save_receipt_to_pdf_error'),
      );
    }
  }
